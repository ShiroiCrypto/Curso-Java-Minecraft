...
public class RaioCommand implements CommandExecutor {
    
    private final Curso plugin;
    private final PoderManager poderManager;
    
    public RaioCommand(Curso plugin, PoderManager poderManager) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!player.hasPermission("superpoderes.raio")) {
            player.sendMessage("§cVocê não tem permissão para invocar raios!");
            return true;
        }
