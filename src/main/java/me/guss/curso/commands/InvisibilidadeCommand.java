...
public class InvisibilidadeCommand implements CommandExecutor {
    
    private final Curso plugin;
    private final PoderManager poderManager;
    
    public InvisibilidadeCommand(Curso plugin, PoderManager poderManager) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!player.hasPermission("superpoderes.invisibilidade")) {
            player.sendMessage("§cVocê não tem permissão para ficar invisível!");
            return true;
        }
