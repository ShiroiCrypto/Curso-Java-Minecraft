...
public class CuraCommand implements CommandExecutor {
    
    private final Curso plugin;
    private final PoderManager poderManager;
    
    public CuraCommand(Curso plugin, PoderManager poderManager) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!player.hasPermission("superpoderes.cura")) {
            player.sendMessage("§cVocê não tem permissão para curar!");
            return true;
        }
